...
const smc = require("cluster-shared-memory");

let osmProcess = require('./old_osm.js');
...
if (cluster.isMaster) {
    ...
    osmProcess.init(true).then(()=>{
        ...
        smc.set('cell', cell);
        smc.set('nodesList', nodeList);
        smc.set('wayList', wayList);
    ...
    (async ()=>{
        ...
        osmProcess.wayList = await smc.get('wayList');
        osmProcess.nodesList = await smc.get('nodesList');
        osmProcess.smc = smc;
        console.log(new Date() - start);
        await osmProcess.init(false);
